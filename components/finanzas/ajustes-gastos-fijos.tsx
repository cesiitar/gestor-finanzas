...
/**
 * Fijos: se repiten igual todos los meses y el cron diario los registra
 * solos el día elegido, avisando por Telegram. Sirve tanto para gastos
 * (alquiler, gym) como para ingresos (nómina), según el tipo que reciba.
 */
export function AjustesGastosFijos({
  ...
  const titulo = esIngreso ? "Ingresos fijos" : "Gastos fijos"
  ...
  const fijosDelTipo = gastosFijos.filter((f) => (f.tipo ?? "gasto") === tipo)
  ...
  const totalFijos = fijosDelTipo.reduce((s, f) => s + (f.activo ? f.importe_cents : 0), 0)
    ...
    <section>
      ...
      {fijosDelTipo.length === 0 ? (
        <div className="flex items-center gap-3 rounded-[1.25rem] border border-dashed border-white/10 px-4 py-5">
          ...
          <p className="text-sm text-neutral-400">
            {esIngreso
              ? "Añade ingresos que entran cada mes (nómina, alquiler que cobras…) y se registrarán solos el día que elijas."
              : "Añade gastos que se repiten cada mes (alquiler, gimnasio…) y se registrarán solos el día que elijas."}
          </p>
        </div>
      ) : (
        <>
          <ul className="space-y-0.5">
            {fijosDelTipo.map((fijo) => (
              <li key={fijo.id}>
                <button
          ...
          <p className="px-1.5 pt-2 text-right text-xs tabular-nums text-neutral-500">
            Total mensual: {formatEUR(totalFijos)}
          </p>
        </>
      )}
      ...
      <Drawer open={abierto} onOpenChange={setAbierto} repositionInputs={false}>
        <DrawerContent className="border-white/[0.08] bg-[#0b0d11]/95 backdrop-blur-2xl shadow-[inset_0_1px_0_rgba(255,255,255,0.10)]">
          <DrawerHeader>
            <DrawerTitle>{editando ? `Editar ${editando.nombre}` : esIngreso ? "Nuevo ingreso fijo" : "Nuevo gasto fijo"}</DrawerTitle>
          </DrawerHeader>
          <form onSubmit={guardar} className="flex flex-col gap-3 px-4 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
            ...
            >
              {editando ? "Guardar cambios" : esIngreso ? "Crear ingreso fijo" : "Crear gasto fijo"}
            </button>
            {editando && (
              <button
                type="button"
